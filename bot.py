...
class MediaDownloader:
    ...
    async def download_media(self, url: str, out_path: str):
        """Универсальная функция для скачивания через yt-dlp"""
        try:
            ...
            raise Exception(f"Не удалось скачать контент: {str(e)}")
    ...
    def _download_with_ytdlp(self, url: str, out_path: str):
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            # Для Instagram определяем более точный тип
            if 'instagram.com' in url:
                if '/stories/' in url:
...
async def start(client, message):
    ...
    try:
        welcome_msg = await message.reply_text(
            "📥 Отправь ссылку на Instagram или YouTube — я скачаю контент:\n\n"
            "• 📹 Видео и рилсы\n" 
            "• 📸 Фото\n"
            "• 🖼️ Карусели\n"
            "• 📖 Истории\n"
...
async def handle_text(client, message):
    ...
    try:
        ...
        if "youtube" in url or "youtu.be" in url:
            ...
            await _handle_instagram(client, message, url, status, downloader, tmp_dir)
...
async def _handle_instagram(client, message, url, status, downloader, tmp_dir):
    """Обработка Instagram через yt-dlp"""
    ...
    try:
        await status.edit_text("⚡ Скачиваю контент Instagram...")
        
        content_info = await downloader.download_media(url, tmp_dir)
...
async def send_content(client, message, content_info):
    ...
    if content_type in ['photo', 'story_photo']:
        ...
        await _send_carousel(client, message, files)
...
if __name__ == "__main__":
    ...
    logger.info("🚀 ЗАПУСК БОТА С yt-dlp...")
    logger.info("📥 Поддерживаемые платформы: Instagram, YouTube")
